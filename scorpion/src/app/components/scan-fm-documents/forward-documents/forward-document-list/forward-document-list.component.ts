    ...
    const payload = {
      wffdmvm: {
        wffdm: {
          ...
          fM_Edit_by: "",
          loc_Cust_Code: this.forwardDocForm.value.Loc_Cust_Code.id || '',
        ...
        ffdfm: {
          docType: this.filterData.DocType,
          fromDate: this.filterData?.dateRange?.[0] ? new Date(this.filterData.dateRange[0]).toISOString() : new Date().toISOString(),
          toDate: this.filterData?.dateRange?.[1] ? new Date(this.filterData.dateRange[1]).toISOString() : new Date().toISOString(),
          paybas: this.filterData.Paybas,
          dockets: this.filterData.Dockets,
          dT_TYPE: this.filterData.DT_TYPE,
          loccode: this.docketService.loginUserList.LocationCode
