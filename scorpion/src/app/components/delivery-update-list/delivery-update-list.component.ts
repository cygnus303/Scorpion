...
resetNewRateOnBlur(index: number): void {
  ...
  this.updateDeliveryValidators(group);
     ...
     this.updateDeliveryValidators(group);
     this.onDeliveredBlur(this.drsList.length - 1);
    });
  }
...
onDeliveredBlur(index: number): void {
  ...
  // 🔹 When delivered packages is 0, nullify DeliveredTo
  if (delivered === 0) {
  ...
  if (show) {
    ...
    // 🔹 Apply validators based on condition
    if (delivered >= pending || delivered === 0) {
      row.get('cboReason')?.setValidators([Validators.required]);
