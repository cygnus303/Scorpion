...
export class PFMListComponent implements OnInit, OnDestroy {
  ...
  statusList = [
    ...
    { value: 'Cancelled', label: 'Cancelled', color: 'cancelled', bg: 'var(--danger)', count: 0 }
  ];
  ...
  formatLRWiseData(data: any[]): any[] {
    return data.map(item => ({
      ...
      'Billing Party': item.billingParty || '',
      ...
      'PFM Acknowledge Date': item.fM_Ack_Date ? this.formatDate(item.fM_Ack_Date) : '',
    }));
  }
  ...
  GetLRData(data: any[]): any[] {
    return data.map(item => ({
      ...
      'PFM No': item.fM_No || '',
      'PFM Date': item.fM_Entry_Date ? this.formatDate(item.fM_Entry_Date) : '',
      'Status': item.fM_Status || '',
      'Since Days': item.daysSince || 0,
      'Billing Party': item.billingParty || '',
      'Courier Name': item.courier_Code || '',
      'Courier date': item.waybilldate ? this.formatDate(item.waybilldate) : '',
      'PFM Generated By': item.entryBy || '',
      'PFM Generated Date': item.fM_Entry_Date ? this.formatDate(item.fM_Entry_Date) : '',
      'PFM Forwarded BY': item.forward_By || '',
      'PFM Forward Date': item.fM_FWD_Date ? this.formatDate(item.fM_FWD_Date) : '',
      'PFM Acknowledge By': item.fM_Ack_By || '',
      'PFM Acknowledge Date': item.fM_Ack_Date ? this.formatDate(item.fM_Ack_Date) : '',
    }));
  }
  ...
  formatPFMReportData(data: any[]): any[] {
    return data.map(item => ({
      ...
      'Courier Name': item.courier_Code || '',
      'Courier date': item.courier_Way_Bill_Date ? this.formatDate(item.courier_Way_Bill_Date) : '',
      'PFM Generated By': item.pfmGeneratedBy || '',
      'PFM Generated Date': item.pfmGeneratedDate ? this.formatDate(item.pfmGeneratedDate) : '',
      'PFM Forwarded BY': item.forward_By || '',
      'PFM Forward Date': item.forwardDate ? this.formatDate(item.forwardDate) : '',
      'PFM Acknowledge By': item.fM_Ack_By || '',
      'PFM Acknowledge Date': item.fM_Ack_Date ? this.formatDate(item.fM_Ack_Date) : '',
  ...
  downloadPFMWiseCSV() {
    ...
    this.PFMapiService.GetPFMReport(payload).subscribe({
      next: (response: any) => {
        this.isCSVLoading = false;
        const csvData = this.formatPFMReportData(response);
        const fileName = `PFM_Wise_List`;
        this.exportService.exportToCSV(csvData, fileName);
      }
    });
  }
