...
export class PFMListComponent implements OnInit, OnDestroy {
  ...
  formatPFMWiseData(data: any[]): any[] {
    return data.map(item => ({
      ...
      'PFM Generated Date': item.pfM_Date ? this.formatDate(item.pfM_Date) : '',
      'PFM Forwarded BY': item.pfM_Forwarded_BY || '',
      'PFM Forward Date': item.pfM_ForwardDate ? this.formatDate(item.pfM_ForwardDate) : '',
      'PFM Acknowledge By': item.pfM_Acknowledge_By || '',
      'PFM Ack date': item.pfM_AckDate ? this.formatDate(item.pfM_AckDate) : '',
      'PFM Cancel By': item.pfM_Cancel_By || '',
      'PFM Cancel Date': item.pfM_CancelDate ? this.formatDate(item.pfM_CancelDate) : '',
    }));
  }
  ...
  cancelPFM(row: any) {
    ...
    //     const payload = {
    //       fmNo: row.fM_No,
    //       cancelBy: this.docketService.loginUserList.UserId
    //     };
