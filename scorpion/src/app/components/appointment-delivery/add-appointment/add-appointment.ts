...
export class AddAppointment implements OnInit {
  ...
  appointmentForm = new FormGroup({
    ...
    appointmentDate: new FormControl<any>(new Date()),
  ...
  validateTimeRange() {
    const timeFromControl = this.appointmentForm.get('timeFrom');
    const timeToControl = this.appointmentForm.get('timeTo');
    const timeFrom = timeFromControl?.value;
    const timeTo = timeToControl?.value;

    if (this.entryType === 'APMT' && timeFrom && timeTo) {
      if (timeTo <= timeFrom) {
        timeToControl?.setErrors({ ...timeToControl?.errors, timeInvalid: true });
        timeToControl?.markAsTouched();
      } else {
        if (timeToControl?.hasError('timeInvalid')) {
          const errors = { ...timeToControl.errors };
          delete errors['timeInvalid'];
          timeToControl.setErrors(Object.keys(errors).length ? errors : null);
        }
      }
    } else if (timeToControl?.hasError('timeInvalid')) {
      const errors = { ...timeToControl.errors };
      delete errors['timeInvalid'];
      timeToControl.setErrors(Object.keys(errors).length ? errors : null);
    }
  ...
  saveCsdMsd(formValue: any) {
    const payload = {
      ...
      baseUserName:this.docketService.loginUserList.UserId.toString()
    };
  ...
  saveApmt(formValue: any) {
    ...
    this.appointmentDeliveryService.addEditAppointment(payload, this.docketService.loginUserList.UserId.toString()).subscribe({
      next: (res) => {
        this.isSaving = false;
        if (res && res.success) {
  ...
  fetchDocketDetails() {
    ...
    this.appointmentDeliveryService.checkDeliveryEligibility(docketNo, entryType).subscribe({
      next: (eligibilityRes: any) => {
        ...
        this.appointmentDeliveryService.getDocDataDetail(docketNo).subscribe({
          next: (res) => {
            this.isDocketLoading = false;
            if (res && res) {
              ...
              this.appointmentForm.patchValue({
                ...
                docketDate: res.documentDate || '',
                edd: res.edd || '',
                consigneeName: res.customerName || '',
                consigneeEmail: res.customerEmail || '',
                consigneeContact: res.contactno || '',
                consigneeAddress: res.csAddress || '',
