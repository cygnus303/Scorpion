import { Component, EventEmitter, Output, ViewChild, inject, TemplateRef } from '@angular/core';
...
export class RescheduleAppointment {
  ...
  @ViewChild('rescheduleModal', { static: true }) rescheduleModal!: TemplateRef<any>;
  ...
  constructor() {
    this.rescheduleForm.get('timeFrom')?.valueChanges.subscribe(() => this.validateTimeRange());
    this.rescheduleForm.get('timeTo')?.valueChanges.subscribe(() => this.validateTimeRange());
  }

  validateTimeRange() {
    const timeFromControl = this.rescheduleForm.get('timeFrom');
    const timeToControl = this.rescheduleForm.get('timeTo');
  ...
  openModal(type: string, data: any) {
    ...
    this.modalRef = this.modalService.show(this.rescheduleModal, {
  ...
  onSubmit() {
    ...
    if (this.rescheduleForm.valid) {
      ...
      this.appointmentService.addEditAppointment(payload, this.docketService.loginUserList.UserId.toString()).subscribe({
        next: (res: any) => {
          this.isSaving = false;
          if (res && res.success) {
